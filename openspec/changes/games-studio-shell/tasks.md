...
- [ ] 3.2 List the Variant designer (`available`, → `/studio/dungeon-tactics/variant`) and the Map editor (`coming-soon`, disabled)

## 4. Variant designer (relocated ScenarioEditor)
...
- [ ] 4.2 Add `VariantDesignerPage` at `/studio/dungeon-tactics/variant` that mounts the (possibly extracted) editor, editing the same Variants via `defStore` + existing `/scenarios/:s/unit-defs` endpoints — no new persistence
