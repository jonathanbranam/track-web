...
The game SHALL display a shake button in the HUD at all times on all devices (not gated on motion availability). The button SHALL be positioned in the top-left of the HUD, below the score display. Tapping it SHALL trigger a jostle immediately if the jostle cooldown has elapsed. The button SHALL be disabled during the cooldown and also when the player's score is below the shake cost. A cost label (e.g. "−50") SHALL be displayed below the button so the player can see the point penalty before tapping.
