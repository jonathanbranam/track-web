...
- [x] 1.4 Remove the now-unused `getEngineMode` import from `actions.ts`, and
      correct the guard's own comment — it currently explains the bench
      exemption.

## 2. The action surface is the player's

- [x] 2.1 `availableActions` reports every action of a `unit.kind === 'npc'`
...
- [x] 2.2 Delete the `npcPlannedThisRound` check in `availableActions`. With 2.1
...
- [x] 2.5 Re-describe 2.3/2.4 in `sequencer.ts`'s comments as **defence-in-depth**
...
- [x] 2.6 `resolveAttack`'s NPC branch: its comment says a hand-driven NPC is
      marked spent so it cannot attack twice, and no hand-driven NPC exists any
      more. `commitAction` cannot reach it; `preview` still does, resolving
      against a copy it discards. Correct the comment; keep the branch, because
      `preview`'s value is that it cannot disagree with resolution.
      *(Found by the apply agent and reported rather than silently changed;
      added to the plan and applied afterwards.)*
...
- [x] 4.1 Out-of-phase acting is refused for a PC — and refused **the same way in
...
- [x] 4.2 An enemy has no actions in any phase, including the player phase, and a
      commit against one is refused. Replaces the old "plan an enemy then drive
      it" test, whose first half no longer has a route.
- [x] 4.3 The sequencer's defence-in-depth guard still holds. No public call can
...
- [x] 5.1 Play a round of the game in a browser (a disposable second instance —
      ...
      *(Done by the apply agent, not the driver: a disposable instance on
      3100/6135, torn down afterwards. It reported a full round — placement,
      Start, a PC move, the "no movement left" reason, End turn with its
      unattacked-units warning, telegraph resolution damaging structures, and the
      chain into the next `npc-move` — plus that clicking an enemy shows an info
      panel with no Move/Attack controls at all. Not independently reconfirmed by
      the driver; the bench side of the same guards was, at length, in 8.2 of
      `harness:dungeon-bench-guard-adoption`.)*
- [x] 5.2 Confirm the refusals read well — an enemy's reason especially, since a
      designer will meet it by clicking one. It should name the planning seat.
- [x] 5.3 The harness will be broken until its own change lands. Do not fix it
