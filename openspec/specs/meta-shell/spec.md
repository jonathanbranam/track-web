...
The `meta-shell` capability provides the talk RPG experience's presentation "bookend" — the cold-open framing screens and achievement toasts (`requirements.md` group H) — built entirely from new action vocabulary consuming the `talk-director`/`ui-overlay`/`world-rendering` capabilities already in place, with no new Director, rendering, or battle primitive. `showSaveFile` reuses the existing `overlay` slot for a distinct "completed, high-level prior playthrough" framing card cleared by the existing `hideOverlay` action; `showAchievement`/`hideAchievement` add an independent `achievement` resting-state field so a toast can pop alongside any active dialogue, menu, or overlay content without displacing it; the title/start screen is demonstrated entirely from already-Established `showOverlay`/`showMenu` vocabulary, requiring no new action type of its own.
