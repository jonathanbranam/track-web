...
Structured packing list per trip. Items are organized by section and managed via API by the trip owner or individual members (for their own personal items). The `packing_items` table supports two scopes: shared items (`user_id IS NULL`, visible to all members) and personal items (`user_id` set, visible only to that user and the owner). Members can add and delete their own personal items directly from the UI; per-user check-off state is handled separately (see `packing-state`).
