...
| `battleAction` | `pc` attacks `slime` for 7 damage | `kind: 'attack' \| 'spell' \| 'item' \| 'wrong-action'` is narratively meaningful but drives no engine/rendering branch by itself — no executor switches on it; `damage` is a **signed** HP delta subtracted from `target`'s HP (clamped to `[0, maxHp]`) — negative heals, which is how `wrong-action` depicts a scripted mistake (e.g. Fire healing a fire-immune enemy); `text` is the authored narration line, shown via the same dialogue-box slot `say` uses; applies uniformly across `battle.allies` and `battle.enemies` |
...
| `partyJoin` | `familiar` joins at named location `'shrine'` | adds a new entity to `world.entities` at an authored named location (or, when `at` is omitted, the protagonist's current position); `TalkRpgScene` detects the join structurally (an entity id present now that wasn't in the prior snapshot, live playback only) and always plays the same fixed scale-in tween — `fx` is authored-but-inert (see below), not a selector; does **not** itself place the entity into battle — a subsequent `startBattle` authors participants explicitly; an authoring convention for genuinely new recruits only, not for entities already present (e.g. previously `tagCombatant('out')`) |
...
### Authored-but-inert fields

Three fields type-check, are legal to author, and in one case are already used in the live `SCRIPT` — but nothing in `precompute.ts`/`executors.ts`/`TalkRpgScene.ts` reads their value today. Authoring a beat with one of these expecting a visible effect will silently do nothing beyond what's noted:

- **`StartBattleAction.surprised`** — not read anywhere; `applyAction`'s `startBattle` case ignores it entirely. It traces back to `script.md`'s `dungeon-ambush` beat ("enemy attacks first because hero was surprised") but no first-strike/turn-order mechanic exists to hang it on yet.
- **`PartyJoinAction.fx`** — not read anywhere; `TalkRpgScene.playJoinEffect` always plays the same hardcoded scale-in tween regardless of the string passed (e.g. `'sparkle'` in the current `SCRIPT`).
- **`BattleActionAction.kind`** — no executor or component branches on it; it's authoring-time documentation of intent only. The actual effect (damage vs. heal, i.e. `wrong-action`'s "mistake") comes entirely from `damage`'s sign.

If/when these get real behavior (ambush turn order, per-`fx` effect variants, per-`kind` animations), update this note and the affected rows above in the same change.
