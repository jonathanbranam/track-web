...
The `entity-stats` capability is the display-oriented entity/stats model for the talk RPG experience: a real, authored `EntityStats` payload (level, optional role, current/max HP) shown via the `showStatus` action on the on-rails status screen established by the `ui-overlay` capability, and a `levelUp` fanfare narration beat for depicting a stat/ability increase. Stats are authored inline per action, not stored in a persistent registry — this framework never computes real combat outcomes or maintains a mutable stats database.
