...
Defines requirements for the Dungeon Tactics Solo heads-up display: the screen-anchored HUD chrome (status pill, action buttons, unit info popup, and end-of-turn confirmation modal) rendered as a ReactDOM overlay over the Phaser canvas. Establishes that interactive HUD controls live in the DOM and invoke the game engine directly, while board-anchored, tile-relative visuals remain rendered by Phaser.
