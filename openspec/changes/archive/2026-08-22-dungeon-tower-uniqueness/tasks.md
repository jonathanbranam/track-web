...
- [x] 1.1 `packages/dungeon-engine/src/turn.ts`: add `towerTiles(cells): Tile[]`
      beside `powerCenterCount`, returning every tower's tile in row-major order.
      Export it from `index.ts` alongside `isTowerImmune`.
- [x] 1.2 `packages/dungeon-engine/src/npc.ts`: replace the planning context's
...
- [x] 2.1 `scenario.placeStructure`: when `kind === 'tower'` and the board
...
- [x] 2.2 Leave `moveStructure` and `removeStructure` alone. If a reviewer would
...
- [x] 3.1 `sequencer.startScenario`: refuse, with a reason, when
      `towerTiles(state.cells).length === 0`. Check it alongside the existing
      phase precondition; state unchanged on refusal.
- [x] 3.2 Confirm the refusal reaches bench mode too — this is not fenced by
      `getEngineMode()`, and a test asserts that.

## 4. The game reports the refusal

- [x] 4.1 `DungeonTacticsGame.tsx`: add `startRefusal: string | null` state.
      ...
      success.
- [x] 4.2 Thread it through `Hud` to `StatusPill` as an optional `refusal` prop.
- [x] 4.3 `StatusPill`: prefer `refusal` over the phase text when present.
      Same placement and styling — no new component.

## 5. Tests

- [x] 5.1 `scenario.test.ts`: a second tower is refused and the board is
...
- [x] 5.2 `sequencer.test.ts`: starting a towerless scenario is refused with a
...
- [x] 5.3 `npc.test.ts`: the planning context still resolves the same tower it
      did before — this is a refactor, and a test should say so.
- [x] 5.4 Re-aim, do not weaken. Existing fixtures that author a board and start
...
- [x] 6.1 `npm test` in track-web and `tsc -b` clean. Paste failures verbatim.
- [x] 6.2 Confirm `bundledMap` still starts — it has one tower at (8, 6) and five
      power centers, so the shipped path is unchanged.
- [x] 6.3 Confirm `isTowerImmune` and the power-center count are untouched.
- [x] 6.4 **Present to the developer. Do NOT commit and do NOT archive.** The
