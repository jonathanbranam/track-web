...
- TV watch progress is tracked as a latest season/episode high-water mark per user
...
- `watch-watchlist`: Per-user state for movies (unseen / watched / would watch again) and TV series (unseen / watching / watched / would watch again), a −2 to 2 rating expressing desire to watch, and latest season/episode progress for TV; event votes propagate to rating when none is set
...
- `src/db.ts` — new tables: `tags`, `movie_tags`, `movie_series`, `movie_series_entries`, `user_movies`, `tv_series`, `tv_series_tags`, `user_tv_series`, `watch_events`, `watch_event_invites`, `watch_event_candidates`, `watch_event_votes`, `watch_event_selection`
