...
export default function MultiEventsDropdown() {
        ...
        <div className="px-4 pt-6">
          ...
          {listTab === 'ratings' && (
            <div className="mb-4 space-y-2.5">
              ...
              {events.length === 0 && (
                <p className="text-xs text-gray-600 italic">No upcoming events</p>
              )}
              {events.length === 1 && (
                <p className="text-xs text-gray-500">
                  Adding to: <span className="text-gray-300">{events[0].name}</span>
                  <span className="text-gray-600 ml-1">· {events[0].date}</span>
                </p>
              )}
              {events.length > 1 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-500 shrink-0">Adding to:</span>
            ...
            <ul className="space-y-2 pb-4">
              {filteredItems.map(item => (
                <li key={item.id} className="bg-gray-800 rounded-2xl p-3">
                  ...
                  {editId !== item.id && activeEvent && (
                    <div className="mt-2">
                      <button
                        onClick={() => addToEvent(item.id, activeEvent.id)}
                        className={`text-xs px-2.5 py-1 rounded-lg transition-colors ${
                          isInEvent(item.id, activeEvent.id)
                            ? 'bg-gray-700/40 text-gray-500 cursor-default'
                            : 'bg-gray-700 text-violet-400 hover:bg-gray-600'
                        }`}
                      >
                        {isInEvent(item.id, activeEvent.id) ? `✓ In ${activeEvent.name}` : `+ ${activeEvent.name}`}
                      </button>
                    </div>
                  )}
                </li>
