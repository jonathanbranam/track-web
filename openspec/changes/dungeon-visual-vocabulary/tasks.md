...
- [ ] 1.1 Add `packages/dungeon-engine/src/palette.ts` exporting exactly what
...
- [ ] 1.2 No import of Phaser, React, or any DOM type in that file. It is data
      plus the `css()` helper and `pipHeightRatio()`; nothing in it draws.
- [ ] 1.3 `css(color)` zero-pads to six hex digits — `css(0x00ff88)` is
      `'#00ff88'`.
- [ ] 1.4 `trianglePoints(r)` returns `[[0, -r], [0.87r, 0.7r], [-0.87r, 0.7r]]`
      — the proportions `renderNpc` uses today. Have `renderNpc` call it (task
      3.3) rather than keeping the literals.
- [ ] 1.4a `pipHeightRatio(maxHp)` is
      `min(maxHeightRatio, (maxColumnRatio - (maxHp - 1) * gapRatio) / maxHp)`,
      per design.md §2.
- [ ] 1.5 Add `SOLICITED_SIDES` and `solicitedSides(phase)` to `palette.ts`, per
      ...
      nothing.
- [ ] 1.6 Add `outlineRole({ phase, side, seats })` to `palette.ts`, returning
      `'live'` exactly when the phase solicits that side and `seats` includes it.
- [ ] 1.7 Export the new surface from `packages/dungeon-engine/src/index.ts`, in
      its own commented section matching the file's existing style.
- [ ] 1.8 Add the `"./palette": "./src/palette.ts"` subpath to
      `packages/dungeon-engine/package.json`'s `exports`, per design.md §7. Keep
      `"."` as it is.
- [ ] 1.9 **`palette.ts` must import only types.** Verify this by reading its
...
- [ ] 2.1 Add `packages/dungeon-engine/src/palette.test.ts` covering: `css()`
...
- [ ] 3.2 `boardRender.ts`: structure pip geometry becomes ratio-derived from
...
- [ ] 3.3 `DungeonTacticsScene.ts`: `UNIT_COLORS` and the overlay colours in
      `drawHighlights` come from the palette. Delete the local tables.
- [ ] 3.4 `DungeonTacticsScene.ts`: `drawHpPips` takes its geometry from `PIP`
      and `pipHeightRatio`, same constraint as 3.2.
- [ ] 3.5 Draw the archetype initial on every unit token, from `UNIT_INITIAL`,
...
- [ ] 3.6 Replace `PC_STROKE` / `NPC_STROKE` / `PC_SELECT_STROKE` with the
...
- [ ] 3.7 A selected unit takes the two-ring mark instead of its live/idle
...
- [ ] 3.8 Leave the spawner, projectile, flash and area colours where they are.
      They are the game's own presentation, and moving them is out of scope.

## 4. Verify and hand back

- [ ] 4.1 `npm test` from the repo root. Paste failures verbatim.
- [ ] 4.2 `npm run build -w client-games` (its `build` is `tsc -b && vite build`,
      so this is the typecheck).
- [ ] 4.3 Report: what changed file by file; exact test output; every test you
      ...
      silently.
- [ ] 4.4 **Do not commit and do not archive.** The driver verifies in a browser
