...
The `client-admin` workspace — the admin console at `admin.branam.us`, restricted to the owner account (user 1). Covers the auth-gated React app, the admin-only access guard, the API-tokens page, the server-side admin guard on `/api/admin/*`, and the deployment wiring for the new client app.
