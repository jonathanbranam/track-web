...
- [x] 1.1 Create `packages/dungeon-engine/src/scenario.ts` with the result types
...
- [x] 1.2 `newScenario(cells)` — bench-mode fence only; returns a state holding
...
- [x] 1.3 Unit ids: an internal `nextUnitId(state, unitType)` deriving
      `<unitType>-<n>` from the highest numeric suffix in `state.units`. No
      module-level counter.
- [x] 1.4 `placeUnit(state, unitType, tile, hp?)` — bounds, occupancy, and
      structure checks; HP defaults to `getMaxHp(unitType)`; returns the placed
      `Unit` alongside the new state.
- [x] 1.5 `removeUnit`, `relocateUnit`, `setUnitHp`, `clearUnits` — the rule
...
- [x] 1.6 `placeStructure(state, kind, tile, hp?)`, `removeStructure`,
...
- [x] 2.1 `index.ts`: `export * as scenario from './scenario'`, plus the two
      result types and the structure-HP defaults, under a section comment saying
      the surface is bench-only.
- [x] 2.2 `engine-mode.ts`: extend the doc comment — the fence now covers two
...
- [x] 3.1 `scenario.test.ts`, setting bench mode in setup and restoring `'game'`
      in teardown.
- [x] 3.2 The fence: every operation refused in game mode; every state-taking
      operation refused in each non-`placement` phase; the two reasons are
      distinguishable; the state is unchanged in both cases.
- [x] 3.3 Units: placed outside every spawn zone; refused on an occupied,
...
- [x] 3.4 Ids: two units of one archetype differ; placing into a state whose ids
      have gaps collides with nothing.
- [x] 3.5 Structures: placed, refused on a unit or another structure, moved with
      HP preserved, removed; and a placed structure changes what `validMoveDests`
      and `threatTiles` report.
- [x] 3.6 An authored scenario passed to `startScenario` enters `npc-move` the
      same way a loaded one does.

## 4. Verify

- [x] 4.1 `npm test` in track-web (unit + Gherkin) and `tsc -b` clean.
- [x] 4.2 Confirm nothing in `client-games` changed and the game's own placement
