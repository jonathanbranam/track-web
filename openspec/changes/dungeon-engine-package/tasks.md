...
- [ ] 1.1 Create `packages/dungeon-engine/` with a `package.json` mirroring
...
- [ ] 1.2 Add `packages/dungeon-engine` to `workspaces` in the root
      `package.json`
- [ ] 1.3 Add `@repo/dungeon-engine: "*"` to `client-games/package.json`
      dependencies and run an install so the workspace link exists

## 2. Move the pure rules modules

- [ ] 2.1 Move `types.ts`, `turn.ts`, `pc.ts`, `npc.ts`, `pathfinding.ts`,
...
- [ ] 2.2 Move their tests — `attackFootprint.test.ts`, `npc.test.ts`,
      `undo.test.ts`, `unitDefs.test.ts`, `placement.test.ts` — alongside them

## 3. Split the definition store

- [ ] 3.1 Move `defStore.ts` into `packages/dungeon-engine/src/`, keeping the
...
- [ ] 3.2 Remove `loadFromServer`, `loadScenario`, `loadedScenario`, the
...
- [ ] 3.3 Create `client-games/src/games/dungeon-tactics-solo/defStoreLoader.ts`
...
- [ ] 3.4 Split `defStore.test.ts`: apply/clamp/diff/read assertions stay with
...
- [ ] 4.1 Move `contentStore.ts` into `packages/dungeon-engine/src/`, keeping
      `deserialize`, the active-content state, `gridCols`/`gridRows`/
      `boardCells`, and the spawn-zone getters
- [ ] 4.2 Remove `loadFromServer` and the `client-games/src/api` import from the
...
- [ ] 4.4 Split `contentStore.test.ts` along the same line — deserialize and
      getter assertions with the package, fetch/fallback assertions with the
      loader

## 5. Define the public surface

- [ ] 5.1 Write `packages/dungeon-engine/src/index.ts` re-exporting the public
...
- [ ] 5.2 Confirm no module inside the package imports from `client-games`

## 6. Update the consumers

- [ ] 6.1 Update `DungeonTacticsScene.ts`, `EditorScene.ts`, `boardRender.ts`,
...
- [ ] 6.2 Update the studio pages — `UnitDesignerPage.tsx`, `MapEditorPage.tsx`,
      `MapEditorHud.tsx`, `MapListPage.tsx` — and
      `unitDesignerPersistence.test.ts` the same way
- [ ] 6.3 Update the Gherkin step definitions under
      `client-games/src/games/dungeon-tactics-solo/features/steps/` to import
      from the package
- [ ] 6.4 Update `editorModel.test.ts` and any remaining test in `client-games`
      that imported a moved module

## 7. Prove it runs in Node

- [ ] 7.1 Add a node-environment test in `packages/dungeon-engine` that imports
...
- [ ] 7.2 Assert in that test that the package touches no browser global, so
      reintroducing `fetch`/`localStorage`/DOM access inside the package fails
      the suite
- [ ] 7.3 Add `packages/dungeon-engine/src/**/*.test.ts` to `include` in the
      root `vitest.config.mts`

## 8. Verify

- [ ] 8.1 `npm test` passes, including every moved and split test
- [ ] 8.2 `npm run test:dungeon-tactics` passes (the Gherkin suite is unchanged
      apart from step-definition imports)
- [ ] 8.3 `npm run build:games` completes with zero TypeScript errors
- [ ] 8.4 Play the game manually: start a match, move and attack with a PC,
      let the enemy phase resolve, and confirm behavior is unchanged
- [ ] 8.5 Open both studio editors — map editor and unit designer — and confirm
