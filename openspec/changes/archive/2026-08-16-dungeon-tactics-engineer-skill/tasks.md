...
- [x] 1.1 Create `.claude/skills/scenario-to-change/SKILL.md` with frontmatter
...
- [x] 1.2 Write the input-handling step: accept a path to a handoff bundle
  directory; if not provided, ask the user for it via `AskUserQuestion`
  (don't guess at a path).
- [x] 1.3 Write the validation step: confirm the bundle directory contains
...
- [x] 2.1 Write the step that runs `openspec new change "<name>"` to create
...
- [x] 2.2 Write the step that copies the bundle's `.feature` file verbatim
...
- [x] 2.3 Write the capability-targeting step: search `openspec/specs/*/spec.md`
...
- [x] 2.4 Write the step that drafts `proposal.md`/`design.md` (following
...
- [x] 2.5 Write the step that drafts the `specs/<capability>/spec.md` delta,
...
- [x] 2.6 Write the step that drafts `tasks.md` with one stub task per
...
- [x] 3.1 Run the skill against the real `melee` handoff bundle at
  ...
  Copied to `/private/tmp/.../scratchpad/melee-handoff/` and walked through
  the skill's steps by hand to produce `openspec/changes/dungeon-tactics-melee-movement/`.
- [x] 3.2 Confirm the resulting change's `features/melee.feature` matches
  the handoff bundle's `melee.feature` byte-for-byte.
  `diff` confirmed identical.
- [x] 3.3 Confirm the resulting `specs/` delta proposes `pc-archetypes` with
  ...
  scenario blocks.
  Confirmed in `specs/pc-archetypes/spec.md`.
- [x] 3.4 Confirm `design.md` includes the friendly-unit-pathing caveat from
  `melee-implementation-notes.md` under a labeled section, and that it did
  not alter any scenario step text.
  Confirmed under "Implementation notes from handoff".
- [x] 3.5 Run `openspec validate --change <resulting-change-id> --strict`
  and confirm it passes with no errors.
  `openspec validate dungeon-tactics-melee-movement --strict` →
  "Change 'dungeon-tactics-melee-movement' is valid".
- [x] 3.6 Delete or archive the scratch verification change once confirmed
  ...
  Deleted `openspec/changes/dungeon-tactics-melee-movement/` — it was a
  temp verification run, not real work to ship.

## 4. Documentation

- [x] 4.1 Add a short note to
