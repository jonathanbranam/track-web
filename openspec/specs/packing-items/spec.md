...
Structured packing list per trip. Items are organized by section and managed via API/CLI by the trip owner. The app renders them read-only; per-user check-off state is handled separately (see `packing-state`).
