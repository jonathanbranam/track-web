...
Covers the two-tier nomination search in the watch event detail page: local catalog search (always-on, low latency) combined with TMDB API search as a second tier. Defines trigger conditions, result layout, duplicate filtering, and the import-then-nominate flow.
