...
Shared UI primitives in `packages/ui` consumed by both `client-time` and `client-watch`. These components have no app-specific logic or styling assumptions; accent colors and layout are supplied by the consuming app.
