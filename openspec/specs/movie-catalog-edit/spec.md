...
Covers the UI for editing an existing movie's metadata from the movie catalog page in the watch app.

## Requirements

### Requirement: Edit an existing movie's definition
The system SHALL allow the user to edit any movie in the catalog from the movie catalog page. Each movie card SHALL display an "Edit" affordance. Activating it SHALL replace the card in-place with an edit form pre-populated with the movie's current title, streaming platform, runtime, description, and selected genre tags. Saving SHALL call `PUT /api/watch/movies/:id` and restore the card view with updated data. Cancelling SHALL restore the original card without making changes.
...
- **THEN** the card is replaced by an edit form with title, streaming, runtime, description, and genre tag toggles all pre-populated from the movie's current data
