...
Defines requirements for the Dungeon Tactics Solo game: a standalone single-player entry in the games registry backed by the existing grid-rendering prototype implementation.
...
The system SHALL locate the Dungeon Tactics Solo source files at `client-games/src/games/dungeon-tactics-solo/`. The component, scene, and model files SHALL be moved from `client-games/src/games/prototypes/grid-rendering/` to this directory with no changes to game logic.
