...
Defines the serializable content model for Dungeon Tactics board content: a Region → Map →
Encounter tree, where a Region owns ordered Maps and carries its own terrain enum and theme, a
Map carries an adjustable size with a matching terrain grid, tile objects, and spawn zones, and
an Encounter holds ordered waves with atomic start triggers and condition lists. The model is
fully serializable to and from JSON so the whole tree can be persisted and reloaded without loss.
