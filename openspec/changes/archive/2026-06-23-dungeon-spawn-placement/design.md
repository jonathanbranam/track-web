...
- The board is `GRID_COLS = 16` × `GRID_ROWS = 8`. Power centers sit at `(8,3)`, `(5,4)`,
...
region "behind" the generators: its center front line is row 4, just behind the forward
`(8,3)` generator, and it recedes toward the back row on the flanks. Structure tiles (the
power centers `(8,3)`, `(5,4)`, `(11,4)`, `(2,6)`, `(14,6)` and the tower `(8,6)`) and the
excluded flank corners are not placeable. NPC starts are all in front of the line and thus
outside the zone.
...
(41 tiles. The forward generator `(8,3)` and its row-3 rank sit in front of the zone; the
structure holes `(8,6)` tower and `(2,6)`/`(14,6)` power centers, and the excluded flank
corners on rows 6–7, are simply absent from the layout.)
...
During `placement` the HUD shows a **Done** button (relabeled "Start"). Pressing it must
**not** open the end-turn confirm modal used in the `player` phase — instead it emits a
distinct event (`hud-placement-done`). The handler in `DungeonTacticsGame.tsx` sets
`phase: 'player'`, leaves PC positions as-is, clears any placement selection, **recomputes
`npcPlans` against the committed PC positions**, and redraws. The recompute is essential:
the plans seeded in `initialState` reflect only the default spawn tiles, so the first
round's enemy actions must be re-derived once the player commits their actual placement —
otherwise NPCs would target where the PCs started, not where they were placed. Combat
overlays now render normally.
...
- **[Encoded layout drifts from the design map]** → The layout is a literal copy of the
  design map; add a unit test asserting `spawnZoneTiles()` equals the 41-tile set above and
