...
export default function MultiEventsChooser() {
  ...
  const [chooserId, setChooserId] = useState<number | null>(null)
  ...
  function addToEvent(itemId: number, eventId: string) {
    if (!isInEvent(itemId, eventId)) {
      setEventCandidates(prev => ({ ...prev, [eventId]: [...(prev[eventId] ?? []), itemId] }))
    }
  }
          ...
          {listTab === 'lists' ? (
            ...
            <ul className="space-y-2 pb-4">
              {filteredItems.map(item => {
                const inAnyEvent = events.some(ev => isInEvent(item.id, ev.id))
                const allInEvent = events.length > 0 && events.every(ev => isInEvent(item.id, ev.id))
                      ...
                      <div className="mt-2">
                        {chooserId === item.id ? (
                            ...
                            {events.map(ev => {
                              ...
                              return (
                                <button
                                  key={ev.id}
                                  onClick={() => {
                                    if (!inEv) addToEvent(item.id, ev.id)
                                    setChooserId(null)
                                  }}
                                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${
                                    inEv
                                      ? 'bg-gray-700/40 border-gray-700 text-gray-500 cursor-default'
                                      : 'bg-violet-900/40 border-violet-800 text-violet-300 hover:bg-violet-900/60'
                                  }`}
                                >
                                  {inEv ? `✓ ${ev.name}` : ev.name}
                                </button>
                              )
                            })}
                            <button
                              onClick={() => setChooserId(null)}
                              className="text-xs text-gray-600 hover:text-gray-400 ml-0.5"
                            >
                              ✕
                            </button>
                          </div>
                          ...
                          <button
                            onClick={() => {
                              if (events.length === 1) {
                                if (!isInEvent(item.id, events[0].id)) addToEvent(item.id, events[0].id)
                              } else if (!allInEvent) {
                                setChooserId(item.id)
                              }
                            }}
                            className={`text-xs px-2.5 py-1 rounded-lg transition-colors ${
                              allInEvent
                                ? 'bg-gray-700/40 text-gray-500 cursor-default'
                                : events.length === 1 && isInEvent(item.id, events[0].id)
                                  ? 'bg-gray-700/40 text-gray-500 cursor-default'
                          ...
                          >
                            {(() => {
                              if (events.length === 1) {
                                return isInEvent(item.id, events[0].id)
                                  ? `✓ In ${events[0].name}`
                                  : `+ ${events[0].name}`
                              }
                              if (allInEvent) return '✓ In all events'
                              if (inAnyEvent) {
                                const count = events.filter(ev => isInEvent(item.id, ev.id)).length
                                return `+ Add to event (${count}/${events.length}) →`
                              }
                              return '+ Add to event →'
                            })()}
                          </button>
