...
The script select screen is the interstitial UI shown on first load of the `engineering-with-ai` talk — it lists available scripts by name, loads the chosen one, and reflects the selection in the URL so the user can navigate back to the list.
