...
The generic `/studio` design-tools section of `client-games` — a top-level route area, separate from the `/game/…` play namespace, that hosts content-authoring tools. Covers the second **"Studio"** bottom-nav tab (shown outside `/game/…`, hidden in-game) and the data-driven hub that lists games offering a studio. Game-agnostic so other games' tools can join later without restructuring.
