...
The `battle` capability depicts a scripted, fully-authored combat sequence for the talk RPG experience: entry into a single reusable battle arena scene (`startBattle`), scripted combat beats that apply authored HP deltas and narration (`battleAction`, including a `wrong-action` kind for depicting mistakes), command issuance via the existing menu shell reused from `ui-overlay`, defeat and outcome sequences (`defeatSequence`/`endBattle`), and minimal per-combatant HP tracking that reconstructs deterministically under `snapTo`/`back`/`skipTo`. There is no combat AI and no computed outcome — every effect is authored on the action itself. Scoped to a single allied combatant; multi-combatant party choreography and real stats are deferred to a later phase.
