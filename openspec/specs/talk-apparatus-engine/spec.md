...
The apparatus engine is the playback engine behind the "AI Eng Dynamic" talk: an authored `BeatAction[]` list run once through a deterministic, headless precompute pass into cached `ApparatusState` checkpoints, plus the presenter controls (`snapTo`/`next`/`back`/`pause`/`resume`/`skipTo`/`skipForward`/`restart`) that operate on that checkpoint array. It shares its control-surface shape with `talk-director`, but unlike that engine's asynchronous, executor-driven playback, `next()` here applies a whole checkpoint segment's state synchronously in one call — the renderer, not the engine, owns visual transition animation via CSS — so `pause()`/`resume()` are no-ops kept only for control-surface parity with `talk-rpg`. It also owns the `BeatAction` vocabulary and rendered-state contract specific to the apparatus diagram: chat log, context window, shelves, gauges/counters, gaze marker, and scene swapping between the apparatus view and the cold-open/close bookend stages.
