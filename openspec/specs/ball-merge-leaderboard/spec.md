...
The Ball Merge HUD SHALL include a quit button that lets the player end an active game voluntarily at any time. Quitting SHALL pause the Phaser scene, submit the current score to the leaderboard server (identical to the game-over path), and display the end-game overlay with the leaderboard. The overlay heading SHALL read "You Quit" (as opposed to "Game Over" for a natural game end) so the player knows which event occurred. The quit button SHALL be hidden once a game has ended (whether by natural overflow or by quitting).
...
#### Scenario: Quitting pauses the scene and submits score
- **WHEN** the player taps the quit button during an active game
- **THEN** the Phaser physics scene is paused, the current score is submitted to `POST /api/scores`, and the end-game overlay appears
...
- **THEN** the leaderboard panel fetches and displays the current top-10, identical to the natural game-over path
