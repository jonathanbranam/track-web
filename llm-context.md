...
- **play** (`play.branam.us`) — in-person game companion (`client-play`): a **Putt** tab (disc-golf/putt-putt scorecard, scoped to the current trip) and a **Score** tab — a general score tracker for tabletop/card games. Score-tracker games are owned by the creating user (the scorekeeper); other users referenced as players get no access. Setup takes a **game name** (from a shared, seeded remembered list — Sushi Go, Tides of Time, Pit, Farkle, Uno — or free-form text, auto-remembered), **players** (picked from the caller's social connections via `GET /api/social/users/connectable`, or free-form names), and an **optional round count** (blank = unlimited). Scores are any integer (negative/zero allowed); running and per-round totals are computed client-side. Games complete by reaching the target, ending early, or a "Done" action on unlimited games; completed games are retained permanently as history (players, scores, timestamps) — **no leaderboard** (deferred). Tables: `score_games(id, user_id, name, target_rounds, status, created_at, completed_at)`, `score_players(id, game_id, user_id, name, position)`, `score_round_scores(game_id, player_id, round_number, value)`, `score_game_names(id, name, name_key)`. API under `/api/play/*`: `GET /game-names`, `GET|POST /score-games`, `GET /score-games/:id`, `PUT|DELETE /score-games/:id/rounds/:roundNumber`, `POST /score-games/:id/complete`.
