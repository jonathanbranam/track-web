...
Defines the "AI Eng Dynamic" talk itself: a new `apparatus` talk kind rendered by the `talk-apparatus-engine` capability, its scene content (cold open, three narrative stages, close), and the three-color visual register used to encode narrative meaning across the apparatus's blocks.
