...
- Add a script, `client-games/scripts/generate-step-catalog.ts`, that walks `client-games/src/games/dungeon-tactics-solo/features/*.feature`, parses each file using `@amiceli/vitest-cucumber`'s own `loadFeature()` (already a devDependency from phase 02 — no new parser dependency needed), and extracts every unique Given/When/Then step text.
...
- **Dependencies:** none added — reuses `@amiceli/vitest-cucumber`, already a devDependency from phase 02.
