...
- **games** (`games.branam.us`) — casual games platform (`client-games`, dev port 6035): Phaser 3 + React, a client-side game registry catalog; games are **Ball Merge** (single-player physics ball-merging) and **Orbital Dodger** (single-player gravity survival), both with server-side leaderboards. Ball Merge has 8 selectable container shapes (levels): `box`, `bowl`, `vase`, `cauldron`, `test-tube`, `diamond`, `hex`, `pit` — the player picks one via a pre-game level picker. Scores are stored in `game_scores(id, user_id, game_slug, mode, level, score, achieved_at)` and the `level` field reflects the player-selected container shape (not always `'box'`). Each level has its own independent leaderboard. API: `POST /api/scores` (submit score), `GET /api/scores/leaderboard?game=&mode=&level=&limit=` (top-N personal bests). Admin CLI: `scores:list`, `scores:clear --confirm`. No local score storage — the server leaderboard is authoritative. **Orbital Dodger** is a real-time single-player game (slug `orbital-dodger`): pilot a ship through a procedurally generated field of planets that exert inverse-square gravity, holding to thrust toward your finger on a finite fuel budget. Score accrues continuously, weighted by proximity to a planet surface, plus flat bonuses for collectible stars; a run ends on planet contact, on drifting out of bounds, on fuel exhaustion, or on quitting. It submits to the same `POST /api/scores` with `mode = 'classic'`; the `level` is `classic` for **Random level** and `level-<id>` for a **saved level** (each saved level has its own leaderboard; test runs from the editor submit nothing). It uses no physics engine: the scene integrates motion itself in fixed sub-steps, since gravity is per-planet inverse-square rather than a uniform field. A tuning panel (⚙, in every build including production) exposes live gravity/thrust/fuel/scoring sliders plus **saved tuning configs**: named snapshots stored server-side in `game_od_configs` and shared by all players. Anyone signed in can create, rename, save over, or delete a config; exactly one is the **Default** (what a player with no selection gets), which can be saved over (the client confirms first: "This will replace the default config for all players") but never renamed or deleted. Stored values are layered over the client's shipped `DEFAULT_TUNING`, so the seeded Default `{}` means the shipped values. Which config a browser plays is remembered in localStorage (`orbital-dodger:config-id`); a missing or deleted selection falls back to the Default. The game waits for the config list before starting (8 s limit, then built-in defaults). On wide screens the open panel sits beside the game — the play area slides left, never shrinks. API: `GET/POST /api/games/orbital-dodger/configs`, `PATCH/DELETE /api/games/orbital-dodger/configs/{id}` (409 duplicate name, 403 rename/delete of the Default). All configs still submit to the same per-level leaderboards. **Saved levels** (`game_od_levels`) are named geometry — a start (a point, or in orbit around one of the level's planets at an angle and direction), planets (position, radius, palette color, optional ring height) and stars — shared by all players and fully independent of configs: the level in play and the config in play are chosen separately, and a level never holds tuning. A **level picker** is shown whenever the game starts (after configs and levels load, 8 s limit each): Random level, Create new, then saved levels by name with Play and Edit; the last level played is pre-selected (`orbital-dodger:level-id` in localStorage, `random` or an id) but never auto-started. Random level is the generated game (stars respawn, starts at center). On a saved level stars don't respawn, the HUD shows how many remain, and collecting the last one ends the run as **Level Complete**. An in-orbit start begins locked on the ring; the first press only starts the run. The in-game **level editor** (from Create new, Edit, or Save as level after a Random run) adds/moves/deletes planets and stars, moves the start, test-plays without submitting, and saves as new or over the level; it warns (never blocks) on start clearance, overlapping planets and rings the current config drops. API: `GET/POST /api/games/orbital-dodger/levels`, `PATCH/DELETE /api/games/orbital-dodger/levels/{id}` (server validates geometry: 1–30 stars, 0–12 planets, positions in the 400 × 720 field; 409 duplicate name).
- **play** (`play.branam.us`) — in-person game companion (`client-play`): a **Putt** tab (disc-golf/putt-putt scorecard, scoped to the current trip) and a **Score** tab — a general score tracker for tabletop/card games. Score-tracker games are owned by the creating user (the scorekeeper); other users referenced as players get no access. Setup takes a **game name** (from a shared, seeded remembered list — Sushi Go, Tides of Time, Pit, Farkle, Uno — or free-form text, auto-remembered), **players** (picked from the caller's social connections via `GET /api/social/users/connectable`, or free-form names; the **creator is added as a default player** — labeled "you", removable, and re-addable), and an **optional round count** (blank = unlimited). Known games have default round counts in a client constant (`client-play/src/gameDefaults.ts` — Sushi Go and Tides of Time default to 3 rounds) that **autofills** an untouched round field on game selection. Scores are any integer (negative/zero allowed); running and per-round totals are computed client-side. Games complete by reaching the target, ending early, or a "Done" action on unlimited games (each **requires confirmation**); an in-progress game can be **discarded** (deletes it). Completed games are retained permanently as history (players, scores, timestamps) and can be **swipe-deleted** from the History list — **no leaderboard** (deferred). While a game is open (play/results views) the floating `UserChip` is hidden. Tables: `score_games(id, user_id, name, target_rounds, status, created_at, completed_at)`, `score_players(id, game_id, user_id, name, position)`, `score_round_scores(game_id, player_id, round_number, value)`, `score_game_names(id, name, name_key)`. API under `/api/play/*`: `GET /game-names`, `GET|POST /score-games`, `GET|DELETE /score-games/:id`, `PUT|DELETE /score-games/:id/rounds/:roundNumber`, `POST /score-games/:id/complete`.
