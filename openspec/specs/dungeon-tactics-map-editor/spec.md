...
Defines the Dungeon Tactics map editor under the studio hub: a list view that lists,
creates, and deletes the region's maps, and an editor view that paints terrain, objects,
and spawn zones onto a Phaser-rendered board with ReactDOM controls, validates the map
against a client mirror of the server schema, and saves through the validated content-write
API. Authored maps are playable directly: PCs seat from the map's `playerSpawnZone` rather
than from any fixed start tiles.
