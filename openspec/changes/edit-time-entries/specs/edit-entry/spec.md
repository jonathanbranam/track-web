...
The system SHALL allow the user to edit the currently running entry by tapping a pencil icon on the running task card. The edit view replaces the card inline and presents a description field and a started-at time picker (with offset buttons). The only actions are Save & Keep Running and Cancel.

#### Scenario: Pencil icon opens edit view
- **WHEN** the user taps the pencil icon on the running task card
- **THEN** the card transitions to an edit view showing the current description pre-filled, the started-at time picker pre-filled with the entry's current started_at, and two buttons: Save & Keep Running and Cancel

#### Scenario: Save & Keep Running patches description and start time
- **WHEN** the user edits the description and/or start time and taps Save & Keep Running
...
- **THEN** the Save & Keep Running button is disabled and an inline error is shown
...
#### Scenario: Cancel in stop-edit mode returns to stop screen
- **WHEN** the user taps Cancel in stop-edit mode
- **THEN** no API call is made and the view returns to the normal stop screen (not the running card)
